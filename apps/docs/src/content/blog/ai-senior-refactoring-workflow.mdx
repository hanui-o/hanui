...
리팩토링한 Claude Code에서 바로 "이거 맞아?" 물어볼 수도 있어요. 근데 저는 일부러 **완전히 다른 도구**로 검증해요. Chrome 브라우저의 Claude 확장 프로그램이요.

이유는 간단해요. **만든 사람한테 검증을 맡기면 안 돼요.**

Claude Chrome 확장은 코드를 보는 게 아니에요. **실제 앱 화면을 봐요.** `localhost:3000`에 띄운 앱을 직접 보면서 체크리스트를 검증해요.
